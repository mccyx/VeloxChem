...
computeCoulombFockDDDD0_FP32(double*         mat_J,
                       ...
                       const uint32_t* screen_cut_ij_tile)
{
    ...
    for (uint32_t m = prec_cut; m < screen_cut; m++)
    {
        ...
        const float  eri_ijkl = Lambda * S_ij_00 * S_kl_00 * (
                    ...
                    + F8_t[0] * 0.125f * inv_S1 * inv_S1 * inv_S2 * (
                    ...
                    + F8_t[0] * 0.125f * inv_S1 * inv_S2 * inv_S2 * (
                    ...
                    + F8_t[0] * 0.0625f * inv_S1 * inv_S1 * inv_S2 * inv_S2 * (
                    ...
                    + F8_t[1] * (-0.125f) * inv_S1 * inv_S1 * inv_S2 * inv_S4 * (

                        +(delta[a0][a1]*delta[b0][b1] + delta[a0][b0]*delta[a1][b1] + delta[a0][b1]*delta[a1][b0])*(delta[c0][c1]*delta[d0][d1] + delta[c0][d0]*delta[c1][d1] + delta[c0][d1]*delta[c1][d0])

                    )

                    + F8_t[1] * (-0.125f) * inv_S1 * inv_S2 * inv_S2 * inv_S4 * (

                        +(delta[a0][a1]*delta[b0][b1] + delta[a0][b0]*delta[a1][b1] + delta[a0][b1]*delta[a1][b0])*(delta[c0][c1]*delta[d0][d1] + delta[c0][d0]*delta[c1][d1] + delta[c0][d1]*delta[c1][d0])

                    )

                    + F8_t[1] * (-0.25f) * inv_S1 * inv_S1 * inv_S4 * (

                        +(QC_0*(QC_1*delta[d0][d1] + QD_0*delta[c1][d1] + QD_1*delta[c1][d0]) + QC_1*(QD_0*delta[c0][d1] + QD_1*delta[c0][d0]) + QD_0*QD_1*delta[c0][c1])*(delta[a0][a1]*delta[b0][b1] + delta[a0][b0]*delta[a1][b1] + delta[a0][b1]*delta[a1][b0])

                    )

                    + F8_t[1] * 0.125f * inv_S1 * inv_S2 * inv_S4 * (
                    ...
                    + F8_t[1] * (-0.25f) * inv_S2 * inv_S2 * inv_S4 * (

                        +(PA_0*(PA_1*delta[b0][b1] + PB_0*delta[a1][b1] + PB_1*delta[a1][b0]) + PA_1*(PB_0*delta[a0][b1] + PB_1*delta[a0][b0]) + PB_0*PB_1*delta[a0][a1])*(delta[c0][c1]*delta[d0][d1] + delta[c0][d0]*delta[c1][d1] + delta[c0][d1]*delta[c1][d0])

                    )

                    + F8_t[1] * (-0.5f) * S1 * inv_S2 * inv_S2 * inv_S4 * (

                        +PA_0*PA_1*PB_0*PB_1*(delta[c0][c1]*delta[d0][d1] + delta[c0][d0]*delta[c1][d1] + delta[c0][d1]*delta[c1][d0])

                    )

                    + F8_t[1] * (-0.5f) * S2 * inv_S1 * inv_S1 * inv_S4 * (

                        +QC_0*QC_1*QD_0*QD_1*(delta[a0][a1]*delta[b0][b1] + delta[a0][b0]*delta[a1][b1] + delta[a0][b1]*delta[a1][b0])

                    )

                    + F8_t[1] * 0.25f * inv_S1 * inv_S4 * (
                    ...
                    + F8_t[2] * 0.125f * S1 * inv_S2 * inv_S2 * inv_S4 * inv_S4 * (
...
__global__ void __launch_bounds__(TILE_SIZE_J)
computeCoulombFockDDDD1_FP32(double*         mat_J,
                       const float*    d_prim_info,
                       const uint32_t  d_prim_count,
                       const float*    dd_mat_D,
                       const uint32_t* dd_first_inds_local,
                       ...
                       const uint32_t* screen_cut_ij_tile)
{
    ...
    for (uint32_t m = prec_cut; m < screen_cut; m++)
    {
        ...
        const float  eri_ijkl = Lambda * S_ij_00 * S_kl_00 * (
                    ...
                    + F8_t[2] * 0.125f * S2 * inv_S1 * inv_S1 * inv_S4 * inv_S4 * (

                        +(QC_0*(QC_1*delta[d0][d1] + QD_0*delta[c1][d1] + QD_1*delta[c1][d0]) + QC_1*(QD_0*delta[c0][d1] + QD_1*delta[c0][d0]) + QD_0*QD_1*delta[c0][c1])*(delta[a0][a1]*delta[b0][b1] + delta[a0][b0]*delta[a1][b1] + delta[a0][b1]*delta[a1][b0])

                    )

                    + F8_t[2] * 0.125f * inv_S1 * inv_S4 * inv_S4 * (
                    ...
                    + F8_t[2] * 0.125f * inv_S2 * inv_S4 * inv_S4 * (
...
__global__ void __launch_bounds__(TILE_SIZE_J)
computeCoulombFockDDDD2_FP32(double*         mat_J,
                       const float*    d_prim_info,
                       const uint32_t  d_prim_count,
                       const float*    dd_mat_D,
                       const uint32_t* dd_first_inds_local,
...
__global__ void __launch_bounds__(TILE_SIZE_J)
computeCoulombFockDDDD3_FP32(double*         mat_J,
                       const float*    d_prim_info,
                       const uint32_t  d_prim_count,
                       const float*    dd_mat_D,
                       const uint32_t* dd_first_inds_local,
                       ...
                       const uint32_t* screen_cut_ij_tile)
{
    ...
    for (uint32_t m = prec_cut; m < screen_cut; m++)
    {
        ...
        const float  eri_ijkl = Lambda * S_ij_00 * S_kl_00 * (
                    ...
                    + F8_t[2] * 0.25f * S2 * inv_S1 * inv_S4 * inv_S4 * (
                    ...
                    + F8_t[2] * 0.5f * S1 * S1 * inv_S2 * inv_S4 * inv_S4 * (
                    ...
                    + F8_t[2] * 0.5f * S2 * S2 * inv_S1 * inv_S4 * inv_S4 * (

                        +QC_0*QC_1*QD_0*QD_1*(-PA_0*(PQ[a1]*delta[b0][b1] + PQ[b0]*delta[a1][b1] + PQ[b1]*delta[a1][b0]) - PA_1*(PQ[a0]*delta[b0][b1] + PQ[b0]*delta[a0][b1] + PQ[b1]*delta[a0][b0]) - PB_0*(PQ[a0]*delta[a1][b1] + PQ[a1]*delta[a0][b1] + PQ[b1]*delta[a0][a1]) - PB_1*(PQ[a0]*delta[a1][b0] + PQ[a1]*delta[a0][b0] + PQ[b0]*delta[a0][a1]) + PQ[a0]*PQ[a1]*delta[b0][b1] + PQ[a0]*PQ[b0]*delta[a1][b1] + PQ[a0]*PQ[b1]*delta[a1][b0] + PQ[a1]*PQ[b0]*delta[a0][b1] + PQ[a1]*PQ[b1]*delta[a0][b0] + PQ[b0]*PQ[b1]*delta[a0][a1])

                    )

                    + F8_t[2] * 0.5f * S2 * inv_S4 * inv_S4 * (
...
__global__ void __launch_bounds__(TILE_SIZE_J)
computeCoulombFockDDDD4_FP32(double*         mat_J,
                       const float*    d_prim_info,
                       const uint32_t  d_prim_count,
                       const float*    dd_mat_D,
                       const uint32_t* dd_first_inds_local,
                       ...
                       const uint32_t* screen_cut_ij_tile)
{
    ...
    for (uint32_t m = prec_cut; m < screen_cut; m++)
    {
        ...
        const float  eri_ijkl = Lambda * S_ij_00 * S_kl_00 * (
                    ...
                    + F8_t[2] * 0.0625f * inv_S1 * inv_S1 * inv_S4 * inv_S4 * (

                        +(delta[a0][a1]*delta[b0][b1] + delta[a0][b0]*delta[a1][b1] + delta[a0][b1]*delta[a1][b0])*(delta[c0][c1]*delta[d0][d1] + delta[c0][d0]*delta[c1][d1] + delta[c0][d1]*delta[c1][d0])

                    )

                    + F8_t[2] * 0.0625f * inv_S1 * inv_S2 * inv_S4 * inv_S4 * (
                    ...
                    + F8_t[2] * 0.0625f * inv_S2 * inv_S2 * inv_S4 * inv_S4 * (
...
__global__ void __launch_bounds__(TILE_SIZE_J)
computeCoulombFockDDDD5_FP32(double*         mat_J,
                       const float*    d_prim_info,
                       const uint32_t  d_prim_count,
                       const float*    dd_mat_D,
                       const uint32_t* dd_first_inds_local,
                       ...
                       const uint32_t* screen_cut_ij_tile)
{
    ...
    for (uint32_t m = prec_cut; m < screen_cut; m++)
    {
        ...
        const float  eri_ijkl = Lambda * S_ij_00 * S_kl_00 * (
                    ...
                    + F8_t[3] * 0.0625f * inv_S1 * inv_S4 * inv_S4 * inv_S4 * (
                    ...
                    + F8_t[3] * 0.0625f * inv_S2 * inv_S4 * inv_S4 * inv_S4 * (
                    ...
                    + F8_t[3] * 0.125f * S1 * inv_S2 * inv_S4 * inv_S4 * inv_S4 * (
...
__global__ void __launch_bounds__(TILE_SIZE_J)
computeCoulombFockDDDD6_FP32(double*         mat_J,
                       const float*    d_prim_info,
                       const uint32_t  d_prim_count,
                       const float*    dd_mat_D,
                       const uint32_t* dd_first_inds_local,
                       ...
                       const uint32_t* screen_cut_ij_tile)
{
    ...
    for (uint32_t m = prec_cut; m < screen_cut; m++)
    {
        ...
        const float  eri_ijkl = Lambda * S_ij_00 * S_kl_00 * (
                    ...
                    + F8_t[3] * (-0.125f) * S2 * inv_S1 * inv_S4 * inv_S4 * inv_S4 * (
...
__global__ void __launch_bounds__(TILE_SIZE_J)
computeCoulombFockDDDD7_FP32(double*         mat_J,
                       const float*    d_prim_info,
                       const uint32_t  d_prim_count,
                       const float*    dd_mat_D,
                       const uint32_t* dd_first_inds_local,
                       ...
                       const uint32_t* screen_cut_ij_tile)
{
    ...
    for (uint32_t m = prec_cut; m < screen_cut; m++)
    {
        ...
        const float  eri_ijkl = Lambda * S_ij_00 * S_kl_00 * (
                    ...
                    + F8_t[3] * 0.125f * inv_S4 * inv_S4 * inv_S4 * (
...
__global__ void __launch_bounds__(TILE_SIZE_J)
computeCoulombFockDDDD8_FP32(double*         mat_J,
                       const float*    d_prim_info,
                       const uint32_t  d_prim_count,
                       const float*    dd_mat_D,
                       const uint32_t* dd_first_inds_local,
                       ...
                       const uint32_t* screen_cut_ij_tile)
{
    ...
    for (uint32_t m = prec_cut; m < screen_cut; m++)
    {
        ...
        const float  eri_ijkl = Lambda * S_ij_00 * S_kl_00 * (
                    ...
                    + F8_t[3] * 0.25f * S1 * S1 * inv_S2 * inv_S4 * inv_S4 * inv_S4 * (
...
__global__ void __launch_bounds__(TILE_SIZE_J)
computeCoulombFockDDDD9_FP32(double*         mat_J,
                       const float*    d_prim_info,
                       const uint32_t  d_prim_count,
                       const float*    dd_mat_D,
                       const uint32_t* dd_first_inds_local,
                       ...
                       const uint32_t* screen_cut_ij_tile)
{
    ...
    for (uint32_t m = prec_cut; m < screen_cut; m++)
    {
        ...
        const float  eri_ijkl = Lambda * S_ij_00 * S_kl_00 * (
                    ...
                    + F8_t[3] * 0.25f * S1 * inv_S4 * inv_S4 * inv_S4 * (
                    ...
                    + F8_t[3] * (-0.25f) * S2 * S2 * inv_S1 * inv_S4 * inv_S4 * inv_S4 * (
...
__global__ void __launch_bounds__(TILE_SIZE_J)
computeCoulombFockDDDD10_FP32(double*         mat_J,
                       const float*    d_prim_info,
                       const uint32_t  d_prim_count,
                       const float*    dd_mat_D,
                       const uint32_t* dd_first_inds_local,
                       ...
                       const uint32_t* screen_cut_ij_tile)
{
    ...
    for (uint32_t m = prec_cut; m < screen_cut; m++)
    {
        ...
        const float  eri_ijkl = Lambda * S_ij_00 * S_kl_00 * (
                    ...
                    + F8_t[3] * 0.25f * S2 * inv_S4 * inv_S4 * inv_S4 * (
...
__global__ void __launch_bounds__(TILE_SIZE_J)
computeCoulombFockDDDD11_FP32(double*         mat_J,
                       const float*    d_prim_info,
                       const uint32_t  d_prim_count,
                       const float*    dd_mat_D,
                       const uint32_t* dd_first_inds_local,
                       ...
                       const uint32_t* screen_cut_ij_tile)
{
    ...
    for (uint32_t m = prec_cut; m < screen_cut; m++)
    {
        ...
        const float  eri_ijkl = Lambda * S_ij_00 * S_kl_00 * (
                    ...
                    + F8_t[3] * (-0.5f) * S1 * S1 * S1 * inv_S2 * inv_S4 * inv_S4 * inv_S4 * (
                    ...
                    + F8_t[3] * 0.5f * S1 * S1 * inv_S4 * inv_S4 * inv_S4 * (
                    ...
                    + F8_t[3] * 0.5f * S1 * S2 * inv_S4 * inv_S4 * inv_S4 * (
...
__global__ void __launch_bounds__(TILE_SIZE_J)
computeCoulombFockDDDD12_FP32(double*         mat_J,
                       const float*    d_prim_info,
                       const uint32_t  d_prim_count,
                       const float*    dd_mat_D,
                       const uint32_t* dd_first_inds_local,
                       ...
                       const uint32_t* screen_cut_ij_tile)
{
    ...
    for (uint32_t m = prec_cut; m < screen_cut; m++)
    {
        ...
        const float  eri_ijkl = Lambda * S_ij_00 * S_kl_00 * (
                    ...
                    + F8_t[3] * (-0.5f) * S2 * S2 * S2 * inv_S1 * inv_S4 * inv_S4 * inv_S4 * (
                    ...
                    + F8_t[3] * 0.5f * S2 * S2 * inv_S4 * inv_S4 * inv_S4 * (
...
__global__ void __launch_bounds__(TILE_SIZE_J)
computeCoulombFockDDDD13_FP32(double*         mat_J,
                       const float*    d_prim_info,
                       const uint32_t  d_prim_count,
                       const float*    dd_mat_D,
                       const uint32_t* dd_first_inds_local,
                       ...
                       const uint32_t* screen_cut_ij_tile)
{
    ...
    for (uint32_t m = prec_cut; m < screen_cut; m++)
    {
        ...
        const float  eri_ijkl = Lambda * S_ij_00 * S_kl_00 * (
                    ...
                    + F8_t[4] * 0.125f * S1 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * (
...
__global__ void __launch_bounds__(TILE_SIZE_J)
computeCoulombFockDDDD14_FP32(double*         mat_J,
                       const float*    d_prim_info,
                       const uint32_t  d_prim_count,
                       const float*    dd_mat_D,
                       const uint32_t* dd_first_inds_local,
                       ...
                       const uint32_t* screen_cut_ij_tile)
{
    ...
    for (uint32_t m = prec_cut; m < screen_cut; m++)
    {
        ...
        const float  eri_ijkl = Lambda * S_ij_00 * S_kl_00 * (
                    ...
                    + F8_t[4] * 0.125f * S2 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * (
...
__global__ void __launch_bounds__(TILE_SIZE_J)
computeCoulombFockDDDD15_FP32(double*         mat_J,
                       const float*    d_prim_info,
                       const uint32_t  d_prim_count,
                       const float*    dd_mat_D,
                       const uint32_t* dd_first_inds_local,
                       ...
                       const uint32_t* screen_cut_ij_tile)
{
    ...
    for (uint32_t m = prec_cut; m < screen_cut; m++)
    {
        ...
        const float  eri_ijkl = Lambda * S_ij_00 * S_kl_00 * (
                    ...
                    + F8_t[4] * 0.25f * S1 * S1 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * (
...
__global__ void __launch_bounds__(TILE_SIZE_J)
computeCoulombFockDDDD16_FP32(double*         mat_J,
                       const float*    d_prim_info,
                       const uint32_t  d_prim_count,
                       const float*    dd_mat_D,
                       const uint32_t* dd_first_inds_local,
                       ...
                       const uint32_t* screen_cut_ij_tile)
{
    ...
    for (uint32_t m = prec_cut; m < screen_cut; m++)
    {
        ...
        const float  eri_ijkl = Lambda * S_ij_00 * S_kl_00 * (
                    ...
                    + F8_t[4] * 0.25f * S1 * S2 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * (
...
__global__ void __launch_bounds__(TILE_SIZE_J)
computeCoulombFockDDDD17_FP32(double*         mat_J,
                       const float*    d_prim_info,
                       const uint32_t  d_prim_count,
                       const float*    dd_mat_D,
                       const uint32_t* dd_first_inds_local,
                       ...
                       const uint32_t* screen_cut_ij_tile)
{
    ...
    for (uint32_t m = prec_cut; m < screen_cut; m++)
    {
        ...
        const float  eri_ijkl = Lambda * S_ij_00 * S_kl_00 * (
                    ...
                    + F8_t[4] * 0.25f * S2 * S2 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * (
...
__global__ void __launch_bounds__(TILE_SIZE_J)
computeCoulombFockDDDD18_FP32(double*         mat_J,
                       const float*    d_prim_info,
                       const uint32_t  d_prim_count,
                       const float*    dd_mat_D,
                       const uint32_t* dd_first_inds_local,
                       ...
                       const uint32_t* screen_cut_ij_tile)
{
    ...
    for (uint32_t m = prec_cut; m < screen_cut; m++)
    {
        ...
        const float  eri_ijkl = Lambda * S_ij_00 * S_kl_00 * (
                    ...
                    + F8_t[4] * 0.5f * S1 * S1 * S1 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * (
...
__global__ void __launch_bounds__(TILE_SIZE_J)
computeCoulombFockDDDD19_FP32(double*         mat_J,
                       const float*    d_prim_info,
                       const uint32_t  d_prim_count,
                       const float*    dd_mat_D,
                       const uint32_t* dd_first_inds_local,
                       ...
                       const uint32_t* screen_cut_ij_tile)
{
    ...
    for (uint32_t m = prec_cut; m < screen_cut; m++)
    {
        ...
        const float  eri_ijkl = Lambda * S_ij_00 * S_kl_00 * (
                    ...
                    + F8_t[4] * 0.5f * S1 * S1 * S2 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * (
                    ...
                    + F8_t[4] * 0.5f * S1 * S2 * S2 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * (
                    ...
                    + F8_t[4] * 0.5f * S2 * S2 * S2 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * (
...
__global__ void __launch_bounds__(TILE_SIZE_J)
computeCoulombFockDDDD20_FP32(double*         mat_J,
                       const float*    d_prim_info,
                       const uint32_t  d_prim_count,
                       const float*    dd_mat_D,
                       const uint32_t* dd_first_inds_local,
                       ...
                       const uint32_t* screen_cut_ij_tile)
{
    ...
    for (uint32_t m = prec_cut; m < screen_cut; m++)
    {
        ...
        const float  eri_ijkl = Lambda * S_ij_00 * S_kl_00 * (
                    ...
                    + F8_t[4] * 0.0625f * inv_S4 * inv_S4 * inv_S4 * inv_S4 * (
                       ...
                       const uint32_t* screen_cut_ij_tile)
{
    ...
    for (uint32_t m = prec_cut; m < screen_cut; m++)
    {
        ...
        const float  eri_ijkl = Lambda * S_ij_00 * S_kl_00 * (
                    ...
                    + F8_t[5] * (-0.125f) * S1 * S2 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * (
...
__global__ void __launch_bounds__(TILE_SIZE_J)
computeCoulombFockDDDD22_FP32(double*         mat_J,
                       const float*    d_prim_info,
                       const uint32_t  d_prim_count,
                       const float*    dd_mat_D,
                       const uint32_t* dd_first_inds_local,
                       ...
                       const uint32_t* screen_cut_ij_tile)
{
    ...
    for (uint32_t m = prec_cut; m < screen_cut; m++)
    {
        ...
        const float  eri_ijkl = Lambda * S_ij_00 * S_kl_00 * (
                    ...
                    + F8_t[5] * 0.25f * S1 * S1 * S2 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * (
...
__global__ void __launch_bounds__(TILE_SIZE_J)
computeCoulombFockDDDD23_FP32(double*         mat_J,
                       const float*    d_prim_info,
                       const uint32_t  d_prim_count,
                       const float*    dd_mat_D,
                       const uint32_t* dd_first_inds_local,
                       ...
                       const uint32_t* screen_cut_ij_tile)
{
    ...
    for (uint32_t m = prec_cut; m < screen_cut; m++)
    {
        ...
        const float  eri_ijkl = Lambda * S_ij_00 * S_kl_00 * (
                    ...
                    + F8_t[5] * 0.25f * S1 * S2 * S2 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * (
...
__global__ void __launch_bounds__(TILE_SIZE_J)
computeCoulombFockDDDD24_FP32(double*         mat_J,
                       const float*    d_prim_info,
                       const uint32_t  d_prim_count,
                       const float*    dd_mat_D,
                       const uint32_t* dd_first_inds_local,
                       ...
                       const uint32_t* screen_cut_ij_tile)
{
    ...
    for (uint32_t m = prec_cut; m < screen_cut; m++)
    {
        ...
        const float  eri_ijkl = Lambda * S_ij_00 * S_kl_00 * (
                    ...
                    + F8_t[5] * 0.5f * S1 * S1 * S1 * S2 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * (
                    ...
                    + F8_t[5] * 0.5f * S1 * S1 * S2 * S2 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * (
                    ...
                    + F8_t[5] * (-0.5f) * S1 * S2 * S2 * S2 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * (
...
__global__ void __launch_bounds__(TILE_SIZE_J)
computeCoulombFockDDDD25_FP32(double*         mat_J,
                       const float*    d_prim_info,
                       const uint32_t  d_prim_count,
                       const float*    dd_mat_D,
                       const uint32_t* dd_first_inds_local,
                       ...
                       const uint32_t* screen_cut_ij_tile)
{
    ...
    for (uint32_t m = prec_cut; m < screen_cut; m++)
    {
        ...
        gpu::computeBoysFunction_f(F8_t, S1 * S2 * inv_S4 * r2_PQ, 8, boys_func_table, boys_func_ft);
        ...
        const float  eri_ijkl = Lambda * S_ij_00 * S_kl_00 * (
                    ...
                    + F8_t[6] * 0.5f * S1 * S1 * S1 * S2 * S2 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * (
                    ...
                    + F8_t[6] * 0.5f * S1 * S1 * S2 * S2 * S2 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * (
                    ...
                    + F8_t[6] * 0.25f * S1 * S1 * S2 * S2 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * (
                    ...
                    + F8_t[7] * (-0.5f) * S1 * S1 * S1 * S2 * S2 * S2 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * inv_S4 * (
